FROM maven:3.9-eclipse-temurin AS deps
WORKDIR /build
...
COPY desktop-test-agent/pom.xml desktop-test-agent/pom.xml
...
WORKDIR /app
ARG MODULE
COPY --from=build /build/${MODULE}/target/*-SNAPSHOT.jar app.jar
